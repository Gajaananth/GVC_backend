...
router.get('/receipt/:payment_id', async (req, res) => {
    try {
        ...
        doc.font('Helvetica').text(`Date: ${(0, date_fns_1.format)(new Date(payment.payment_date), 'yyyy-MM-dd')}`, 60);
...
router.get('/statement/:customer_id', async (req, res) => {
    try {
        ...
        doc.text(`Date Generated: ${(0, date_fns_1.format)(new Date(), 'yyyy-MM-dd')}`);
        doc.moveDown(2);
            ...
            loans.forEach(loan => {
                ...
                if (loan.loan_payments && loan.loan_payments.length > 0) {
                    ...
                    loan.loan_payments
                        ...
                        .forEach((p) => {
                        doc.text(`  • ${(0, date_fns_1.format)(new Date(p.payment_date), 'yyyy-MM-dd')}: LKR ${Number(p.amount).toFixed(2)} (${p.payment_type}) - ${p.payment_code}`);
                    });
                }
                else {
...
router.get('/loan-certificate/:loan_id', async (req, res) => {
    try {
        ...
        doc.text(`Settlement Date: ${(0, date_fns_1.format)(new Date(loan.last_payment_date || new Date()), 'yyyy-MM-dd')}`, { align: 'center' });
        doc.moveDown(4);
        const sigY = doc.y;
