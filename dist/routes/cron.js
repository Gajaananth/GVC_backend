...
router.post('/nightly', async (_req, res) => {
    logger_1.logger.info('Starting nightly cron job...');
    const todayDate = (0, date_fns_1.format)(new Date(), 'yyyy-MM-dd');
    try {
        // 1. Mark overdue installments
        const { data: overdueSchedules, error: schError } = await supabase_1.supabase
            .from('loan_schedule')
        ...
        if (schError)
            throw schError;
        logger_1.logger.info(`Marked ${overdueSchedules?.length || 0} installments as overdue.`);
        ...
        if (loanIds.length > 0) {
            await supabase_1.supabase
                ...
                .in('id', loanIds)
                .eq('status', 'active');
            logger_1.logger.info(`Marked ${loanIds.length} loans as overdue.`);
        }
        // 3. Apply late fees on overdue installments that have passed the grace period
        const { data: settings } = await supabase_1.supabase.from('company_settings').select('late_fee_percentage, grace_period_days').single();
        ...
        const graceDateStr = (0, date_fns_1.format)(graceDate, 'yyyy-MM-dd');
        const { data: pastGraceSchedules } = await supabase_1.supabase
            .from('loan_schedule')
            .select('loan_id')
            .eq('status', 'overdue')
        ...
        if (pastGraceLoanIds.length > 0) {
            ...
            if (loans) {
                ...
                logger_1.logger.info(`Applied late fees to ${loans.length} loans.`);
            }
        }
        // 4. Send due date reminders for next 3 days
        const nextThreeDays = new Date();
        nextThreeDays.setDate(nextThreeDays.getDate() + 3);
        const targetDate = (0, date_fns_1.format)(nextThreeDays, 'yyyy-MM-dd');
        const { data: upcomingSchedules } = await supabase_1.supabase
            .from('loan_schedule')
        ...
        if (upcomingSchedules && upcomingSchedules.length > 0) {
            for (const schedule of upcomingSchedules) {
                if (schedule.loans?.customers?.phone) {
                    ...
                    await (0, sms_1.sendSMS)(schedule.loans.customers.phone, message);
                }
            }
            logger_1.logger.info(`Sent ${upcomingSchedules.length} due reminders.`);
        }
        // 5. Add savings interest accrual
        const { data: savingsAccounts } = await supabase_1.supabase
        ...
        if (savingsAccounts && savingsAccounts.length > 0) {
            ...
            if (interestCreditedCount > 0) {
                logger_1.logger.info(`Credited interest to ${interestCreditedCount} savings accounts.`);
            }
        }
        // Log cron execution
        ...
        const { count: activeLoansCount } = await supabase_1.supabase.from('loans').select('id', { count: 'exact', head: true }).eq('status', 'active');
        const { count: overdueLoansCount } = await supabase_1.supabase.from('loans').select('id', { count: 'exact', head: true }).eq('status', 'overdue');
        const { count: pendingApprovalsCount } = await supabase_1.supabase.from('loans').select('id', { count: 'exact', head: true }).eq('approval_status', 'pending_approval');
        // Total collections today
        const { data: todayCollections } = await supabase_1.supabase.from('loan_payments').select('amount').eq('payment_date', todayDate).eq('approval_status', 'approved');
    ...
    `;
        await (0, email_1.sendEmail)('gajaananthnadan17898@gmail.com', `Daily Finance Report - ${todayDate}`, emailHtml);
        res.json({ message: 'Nightly cron job completed successfully' });
    }
    catch (error) {
        logger_1.logger.error('Nightly cron job failed', error);
