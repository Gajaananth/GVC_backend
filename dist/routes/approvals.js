...
router.get('/loans/pending', auth_1.requireOwner, async (req, res) => {
...
router.post('/loans/:id/approve', auth_1.requireOwner, async (req, res) => {
    const { credit_date: creditDateOverride } = req.body || {};
    const { data: loan, error: fetchErr } = await supabase_1.supabase
        .from('loans')
...
router.post('/loans/:id/reject', auth_1.requireOwner, async (req, res) => {
...
router.post('/loans/:id/forward', auth_1.requireBranchManager, async (req, res) => {
    const { data: loan, error: fetchErr } = await supabase_1.supabase
        .from('loans')
        .select('*')
        .eq('id', req.params.id)
...
router.post('/loans/:id/manager-reject', auth_1.requireBranchManager, async (req, res) => {
    const { rejection_reason } = req.body;
    if (!rejection_reason || typeof rejection_reason !== 'string') {
        res.status(400).json({ error: 'Rejection reason is required' });
        return;
...
router.get('/assignments/pending', auth_1.requireOwner, async (_req, res) => {
    const { data, error } = await supabase_1.supabase
...
router.post('/assignments/:id/approve', auth_1.requireOwner, async (req, res) => {
    const { data: change, error: fetchErr } = await supabase_1.supabase
        .from('loan_assignment_changes')
        .select('*')
        .eq('id', req.params.id)
        .eq('status', 'pending_owner')
...
router.post('/assignments/:id/reject', auth_1.requireOwner, async (req, res) => {
    const { data, error } = await supabase_1.supabase
        .from('loan_assignment_changes')
        .update({
        status: 'rejected',
...
router.post('/loans/:id/request-in-charge-change', auth_1.requireAdmin, async (req, res) => {
