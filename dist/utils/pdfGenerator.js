...
exports.generateLoanApplicationPDF = generateLoanApplicationPDF;
exports.uploadLoanFormPDF = uploadLoanFormPDF;
 ...
 */
async function generateLoanApplicationPDF(data) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.font('Helvetica').text(`Application Date: ${(0, date_fns_1.format)(new Date(), 'yyyy-MM-dd')}`, 60);
            ...
            if (data.guarantorName || data.guarantorPhone) {
                ...
                if (data.guarantorName)
                    doc.text(`Name: ${data.guarantorName}`);
                if (data.guarantorPhone)
                    doc.text(`Phone: ${data.guarantorPhone}`);
                if (data.collateralNotes)
                    doc.text(`Collateral: ${data.collateralNotes}`);
            }
            // Staff Details
            doc.moveDown(1);
            ...
            doc.moveDown(4);
            doc.fontSize(9).fillColor('#6b7280').text('This is a computer generated loan application form. The terms are subject to company policy.', { align: 'center' });
 ...
 */
async function uploadLoanFormPDF(loanId, loanCode, pdfBuffer) {
    const storagePath = `loans/${loanId}/loan_form_${loanCode}_${(0, uuid_1.v4)()}.pdf`;
