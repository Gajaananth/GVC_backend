...
const sendSMS = async (to, message) => {
    try {
        ...
        if (!userId || !apiKey) {
            logger_1.logger.warn('Notify.lk credentials missing. Cannot send SMS.');
            return false;
        }
        ...
        if (result.status === 'success') {
            logger_1.logger.info(`SMS sent successfully to ${formattedTo}`);
            return true;
        }
        else {
            logger_1.logger.error('Failed to send SMS:', result);
            return false;
        }
    }
    catch (error) {
