...
export async function applySavingsTransaction(txId: string): Promise<{ success: boolean; error?: string }> {
  ...
  if (!account?.is_active) return { success: false, error: 'Account inactive' };
  ...
  await supabase.from('savings_accounts').update(updateData).eq('id', tx.account_id);

  return { success: true };
}

export async function reverseSavingsTransaction(txId: string): Promise<{ success: boolean; error?: string }> {
