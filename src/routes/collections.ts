...
import { applyLoanPayment, reverseLoanPayment } from '../utils/applyPayment';
import { applySavingsTransaction, reverseSavingsTransaction } from '../utils/applySavings';
...
const staffSavingsSchema = z.object({
...
const reconciliationSchema = z.object({
  ...
  declared_cash_total: z.number().min(0),
  declared_online_total: z.number().min(0),
  admin_notes: z.string().optional().nullable()
});

const correctionSchema = z.object({
  entity_type: z.enum(['loan_payment', 'savings_transaction']),
  entity_id: z.string().uuid(),
  request_type: z.enum(['void', 'amend']),
  letter_description: z.string().min(10),
  proposed_amount: z.number().positive().optional(),
  proposed_cash_amount: z.number().min(0).optional(),
  proposed_online_amount: z.number().min(0).optional(),
  proposed_transaction_date: z.string().optional()
});

const executeCorrectionSchema = z.object({
  amount: z.number().positive().optional(),
  cash_amount: z.number().min(0).optional(),
  online_amount: z.number().min(0).optional(),
  transaction_date: z.string().optional(),
  void_only: z.boolean().optional()
});

async function getStaffDayTotals(staffId: string, date: string) {
...
async function isReconciliationBalanced(staffId: string, date: string): Promise<boolean> {
...
router.post('/submit/savings', requireStaff, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/reconciliation/:staffId/:date', requireAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const { data: staff } = await supabase.from('users').select('id, full_name').eq('id', staffId).single();
...
router.post('/reconciliation', requireAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.post('/payments/:id/approve', requireAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (payment.customers && payment.customers.phone) {
    ...
    await sendSMS(payment.customers.phone, message);
  }
...
router.post('/payments/:id/reject', requireAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
...
router.post('/savings/:id/approve', requireAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  await supabase.from('savings_transactions').update({
    approved_by: req.user!.id,
    approved_at: new Date().toISOString()
  }).eq('id', tx.id);
...
router.post('/savings/:id/reject', requireAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  const { rejection_reason } = req.body;
  const { data, error } = await supabase
    .from('savings_transactions')
    .update({
...
router.post('/corrections', requireStaff, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const body = correctionSchema.parse(req.body);

    const table = body.entity_type === 'loan_payment' ? 'loan_payments' : 'savings_transactions';
    const { data: entity } = await supabase.from(table).select('id, created_by, approval_status').eq('id', body.entity_id).single();
...
router.post('/corrections/:id/approve', requireOwner, async (req: AuthRequest, res: Response): Promise<void> => {
...
router.post('/corrections/:id/reject', requireOwner, async (req: AuthRequest, res: Response): Promise<void> => {
  const { data, error } = await supabase
    .from('collection_correction_requests')
    .update({
      status: 'rejected',
...
router.post('/corrections/:id/execute', requireAdminOrOwner, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (request.request_type === 'void' || body.void_only) {
      if (request.entity_type === 'loan_payment') {
        await reverseLoanPayment(request.entity_id, req.user!.id);
      } else {
        await reverseSavingsTransaction(request.entity_id);
      }
      ...
      if (request.entity_type === 'loan_payment') {
        await reverseLoanPayment(request.entity_id, req.user!.id);
        const { data: old } = await supabase.from('loan_payments').select('*').eq('id', request.entity_id).single();
        if (old && newAmount) {
          const { data: created } = await supabase.from('loan_payments').insert({
            loan_id: old.loan_id,
          ...
          if (created) await applyLoanPayment(created.id, req.user!.id);
        }
      } else {
        await reverseSavingsTransaction(request.entity_id);
        const { data: old } = await supabase.from('savings_transactions').select('*').eq('id', request.entity_id).single();
        if (old && newAmount) {
          const { data: created } = await supabase.from('savings_transactions').insert({
          ...
          if (created) await applySavingsTransaction(created.id);
        }
      }
    }

    await supabase.from('collection_correction_requests').update({
      status: 'executed',
...
router.get('/daily-dues', async (req: AuthRequest, res: Response): Promise<void> => {
  const date = (req.query.date as string) || format(new Date(), 'yyyy-MM-dd');
  let staffId = req.query.staff_id as string | undefined;
  ...
  let items = (scheduleRows || []).map((row: any) => ({
    schedule_id: row.id,
...
router.get('/my-submissions', requireStaff, async (req: AuthRequest, res: Response): Promise<void> => {
  const { data: payments } = await supabase
    .from('loan_payments')
