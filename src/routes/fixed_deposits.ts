...
const addProfessionalCertificateFrame = (doc: PDFKit.PDFDocument, title: string) => {
  const margin = 40;
  // Double Border
...
const drawSignatures = (doc: PDFKit.PDFDocument, sigY: number) => {
...
router.get('/:id/creation-certificate', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    drawRow('Customer Name:', fd.customers.full_name, currY); currY += 15;
    drawRow('NIC Number:', fd.customers.nic_number, currY); currY += 15;
    drawRow('Address:', fd.customers.address || '-', currY); currY += 15;
    drawRow('Phone Number:', fd.customers.phone_number || '-', currY); currY += 25;

    doc.fontSize(12).font('Helvetica-Bold').fillColor('#166534').text('Fixed Deposit Information', 60, currY);
    currY += 20;
    doc.fontSize(10).fillColor('#000000');
    drawRow('Deposit Date:', new Date(fd.created_at).toLocaleDateString(), currY); currY += 15;
    drawRow('Principal Amount:', `${settings.currency_symbol} ${Number(fd.principal_amount).toLocaleString()}`, currY); currY += 15;
    drawRow('Interest Rate:', `${fd.interest_rate}% p.a.`, currY); currY += 15;
    drawRow('Deposit Period:', `${fd.term_months} Months`, currY); currY += 15;
    drawRow('Maturity Date:', new Date(fd.maturity_date).toLocaleDateString(), currY); currY += 15;
    drawRow('Expected Maturity Value:', `${settings.currency_symbol} ${Number(fd.total_maturity_amount).toLocaleString()}`, currY); currY += 30;
...
router.get('/:id/early-closure-certificate', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    drawRow('Customer Name:', fd.customers.full_name, currY); currY += 15;
    drawRow('NIC Number:', fd.customers.nic_number, currY); currY += 15;
    drawRow('Address:', fd.customers.address || '-', currY); currY += 25;
    ...
    drawRow('Interest Rate:', `${fd.interest_rate}% p.a.`, currY); currY += 15;
    drawRow('Deposit Date:', new Date(fd.created_at).toLocaleDateString(), currY); currY += 15;
    drawRow('Original Maturity Date:', new Date(fd.maturity_date).toLocaleDateString(), currY); currY += 25;
    ...
    currY += 20;
    doc.fontSize(10);
    drawRow('Closure Date:', new Date(fd.closed_at || new Date()).toLocaleDateString(), currY); currY += 15;
    drawRow('Interest Earned:', `${settings.currency_symbol} ${interestEarned.toLocaleString()}`, currY); currY += 15;
    if (penalty > 0) {
      drawRow('Penalty Deducted:', `${settings.currency_symbol} ${penalty.toLocaleString()}`, currY, true); currY += 15;
    }
...
router.get('/:id/maturity-closure-certificate', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    drawRow('Deposit Date:', new Date(fd.created_at).toLocaleDateString(), currY); currY += 15;
    drawRow('Maturity Date:', new Date(fd.maturity_date).toLocaleDateString(), currY); currY += 15;
    drawRow('Deposit Period:', `${fd.term_months} Months`, currY); currY += 25;
    ...
    currY += 20;
    doc.fontSize(10);
    drawRow('Settlement Date:', new Date(fd.closed_at || new Date()).toLocaleDateString(), currY); currY += 15;
    drawRow('Total Interest Earned:', `${settings.currency_symbol} ${totalInterest.toLocaleString()}`, currY); currY += 15;
