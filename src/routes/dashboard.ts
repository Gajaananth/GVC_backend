...
router.get('/summary', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (user.role !== 'owner') pendingLoansQuery = pendingLoansQuery.eq('branch_id', user.branch_id);
  ...
  if (user.role !== 'owner') pendingAssignmentsQuery = pendingAssignmentsQuery.eq('branch_id', user.branch_id);
  ...
  let pendingCorrectionsQuery = supabase
    ...
    .select('*', { count: 'exact', head: true })
    .eq('status', 'pending_owner');
  if (user.role !== 'owner') pendingCorrectionsQuery = pendingCorrectionsQuery.eq('branch_id', user.branch_id);
  ...
  if (user.role !== 'owner') pendingPhysicalFormsQuery = pendingPhysicalFormsQuery.eq('branch_id', user.branch_id);
